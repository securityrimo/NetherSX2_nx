/* imports.c -- libemucore.so import resolution (338 undefined symbols)
 *
 * Every undefined dynamic symbol of libemucore.so is bound here. EGL goes
 * through the thin mesa hooks (hooks/egl.c); GL is NOT in this table -- the core
 * loads all gl* dynamically via eglGetProcAddress (GLAD). Audio -> the AAudio
 * shim (aaudio.c). Threads/locale/fs/fortify -> the bionic shims (libc_shim.c,
 * pthr.c). dlopen/dlsym -> dl_emu.c. The rest -> newlib.
 *
 * This table was generated by categorizing the .so's dynsym (scratchpad/
 * gen_imports.py) and hand-reviewed. so_resolve runs with taint=1, so any symbol
 * that slips through aborts ONLY if actually called -- surfacing gaps on-device.
 *
...
extern so_module emu_mod; // libemucore.so (main.c) -- for abort/exit caller offset
...
int __android_log_print(int prio, const char *tag, const char *fmt, ...) {
  (void)prio;
#ifdef DEBUG_LOG
  va_list va; static char buf[0x1000];
...
// PCSX2's Console/logging (incl. the boot/CDVD/ELF/eeloadHook messages) goes
// through these -- previously ret0 (dropped), so the core's own boot diagnostics
// never reached debug.log. Route them through too.
...
static int sigaction_fake(int s, const void *act, void *old) { (void)s; (void)act; (void)old; return 0; }
static int sigemptyset_fake(void *set) { (void)set; return 0; }
...
// clock_nanosleep isn't in newlib; emulate over nanosleep. ★ CRITICAL BUG FIXED:
// the core is built for Android where TIMER_ABSTIME==1, but newlib's <time.h>
// defines TIMER_ABSTIME as 4. So `flags & TIMER_ABSTIME` was FALSE for the frame
// limiter (which passes flags=1) -> we fell through to the relative path and slept
// for the ABSOLUTE deadline value (~40s) as if relative -> the emu thread froze
// (svcGetThreadContext caught it parked in emucore+0x702dc8, PCSX2's frame limiter).
// Detect "absolute" via bit0 (the value the core actually passes), compute the
// delta from our monotonic clock, and CAP the resulting sleep: the limiter asks
// for tens of seconds during the light BIOS boot (emulated clock races ahead of
// real time); we're far below real-time speed anyway, so under-throttling is fine.
static int clock_nanosleep_fake(clockid_t clk, int flags, const struct timespec *req,
  ...
  if (d.tv_sec > 0 || d.tv_nsec > 33000000L) { // cap at ~2 frames (60fps)
    ...
    if ((ct++ & 0xfff) == 0)
      debugPrintf("clock_nanosleep: capped sleep %lld.%09ld s -> 33ms (flags=%d #%lu)\n",
                  (long long)d.tv_sec, d.tv_nsec, flags, ct);
...
// abort()/exit() loggers: the core terminates the process on a failed assertion
// (no CPU fault, so the exception handler won't fire). Log the CALLER's address
// (emucore+offset pinpoints the assertion site) before terminating.
static void emucore_off(char *b, size_t n, uintptr_t a) {
  uintptr_t base = (uintptr_t)emu_mod.load_virtbase, end = base + emu_mod.load_size;
  if (a >= base && a < end) snprintf(b, n, "emucore+0x%lx", (unsigned long)(a - base));
  else snprintf(b, n, "0x%lx", (unsigned long)a);
}
static void abort_fake(void) {
  char w[48]; emucore_off(w, sizeof(w), (uintptr_t)__builtin_return_address(0));
  debugPrintf("*** core called abort() from %s ***\n", w);
  extern void NX_NORETURN __libnx_exit(int rc);
  __libnx_exit(134);
}

// recompiler thread-local init thunks (_ZTH*). The thread_locals are zero/lazy-
// initialised by the recompiler itself; a no-op suffices for the common case.
// FLAG: if the JIT faults on first use, these may need real TLS-ctor stubs.
